<x-guest-layout>
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-slate-900 tracking-tight">{{ __('Create an account') }}</h2>
        <p class="text-sm text-slate-500 mt-2">{{ __('Enter your details to create your new account') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4" x-data="{ loading: false }" @submit="loading = true">
        ...
        <!-- Name -->
        <div>
            <x-input-label for="name" :value="__('Name')" class="text-zinc-600 text-xs font-medium mb-1" />
            <x-text-input id="name" class="block w-full bg-slate-50/50 border-slate-200 text-slate-900 placeholder-slate-400 focus:bg-white focus:border-teal-500 focus:ring-teal-500 rounded-lg px-3.5 py-2.5 text-sm transition-all duration-200" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" placeholder="John Doe" />
            <x-input-error :messages="$errors->get('name')" class="mt-1" />
        </div>

        <!-- Email Address -->
        <div>
            <x-input-label for="email" :value="__('Email')" class="text-zinc-600 text-xs font-medium mb-1" />
        ...
        <div>
            <x-input-label for="password" :value="__('Password')" class="text-zinc-600 text-xs font-medium mb-1" />
        ...
        <!-- Confirm Password -->
        <div>
            <x-input-label for="password_confirmation" :value="__('Confirm Password')" class="text-zinc-600 text-xs font-medium mb-1" />
            <div class="relative" x-data="{ show: false }">
                <x-text-input id="password_confirmation" class="block w-full bg-slate-50/50 border-slate-200 text-slate-900 placeholder-slate-400 focus:bg-white focus:border-teal-500 focus:ring-teal-500 rounded-lg px-3.5 py-2.5 pr-10 text-sm transition-all duration-200"
                                ::type="show ? 'text' : 'password'"
    ...
    <!-- Login Link -->
    <div class="mt-6 pt-5 border-t border-slate-100 text-center text-xs text-zinc-500">
        {{ __('Already registered?') }}
        <a href="{{ route('login') }}" class="font-semibold text-teal-600 hover:text-teal-500 hover:underline transition-colors duration-150 ms-1">
            {{ __('Log in') }}
        </a>
    </div>
</x-guest-layout>
