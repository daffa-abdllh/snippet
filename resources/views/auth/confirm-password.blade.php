<x-guest-layout>
    <div class="mb-6">
        <h2 class="text-3xl font-bold text-slate-900 tracking-tight leading-tight">{{ __('Confirm Password') }}</h2>
        <p class="text-sm text-slate-500 mt-1.5 leading-relaxed">
            {{ __('This is a secure area of the application. Please confirm your password before continuing.') }}
        </p>
    </div>

    <form method="POST" action="{{ route('password.confirm') }}" class="space-y-4" x-data="{ loading: false }" @submit="loading = true">
        @csrf

        <!-- Password -->
        <div>
            <x-input-label for="password" :value="__('Password')" class="text-slate-600 text-xs font-medium mb-1" />
